...
public abstract class Request<T> implements Comparable<Request<T>> {
    ...
    public interface Method {
        /**
         * 不知道什么方法的时候使用.最后依据是否有参数来决定GET还是POST
         */
    ...
    /**
     * Listener interface for progressing.
     */
    ...
    public Request(int methood, String url, Response.ErrorListener listener) {
        this(methood, url, listener, null);
    }

    /**
     * Creates a new request with the given method (one of the values from {@link com.android.volley.Request.Method}),
     * URL, and error errorListener.  Note that the normal response errorListener is not provided here as
     * delivery of responses is provided by subclasses, who have a better idea of how to deliver
     * an already-parsed response.
     *
     * @param method           请求方法.
     * @param url              请求URL.
     * @param errorListener    错误Listener.
     * @param progressListener 进度Listener.
     */
    public Request(int method, String url, Response.ErrorListener errorListener, Response.ProgressListener progressListener) {
        mMethod = method;
        mUrl = url;
    ...
    /**
     * 如果需要上传,必须重写这个方法传递参数.
     * 其实,最后也是调用 getPostBody()返回 Byte[] 用于上传.
